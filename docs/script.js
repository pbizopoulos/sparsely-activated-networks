...
function getNeuronDescriptionLength(index) {
	return (
		tf
			.min([
				2 * neuronArray[index].activation.data.asType("bool").sum().arraySync(),
		...
		const { value, grads } = tf.variableGrads(() => {
			for (let i = 0; i < neuronMaxNum; i++) {
				if (neuronArray[i].use) {
					...
					neuronActivationAreaAllowedArray[i] = new Array(input.data.size).fill(
						0,
					);
				}
			}
				...
				if (neuronArray[i].use) {
					if (neuronArray[i].activation.regulated) {
						neuronArray[i].activation.data = neuronArray[i].activation.data.mul(
							neuronActivationAreaAllowedPostArray[i],
						);
					}
