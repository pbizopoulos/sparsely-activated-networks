...
if __name__ == '__main__':
    ...
    download_physionet(dataset_name_list, path_cache)
    ...
    for index_dataset_name, (dataset_name, xlim_weights) in enumerate(zip(dataset_name_list, xlim_weights_list)):
        ...
        training_dataset = PhysionetDataset('training', dataset_name, path_cache)
        training_dataloader = DataLoader(dataset=training_dataset, batch_size=batch_size, shuffle=True)
        validation_dataset = PhysionetDataset('validation', dataset_name, path_cache)
        validation_dataloader = DataLoader(dataset=validation_dataset)
        test_dataset = PhysionetDataset('test', dataset_name, path_cache)
    ...
    batch_size = 64
    lr = 0.01
    download_uci_epilepsy(path_cache)
    training_dataset = UCIepilepsyDataset('training', path_cache)
    ...
    validation_dataset = UCIepilepsyDataset('validation', path_cache)
    ...
    test_dataset = UCIepilepsyDataset('test', path_cache)
    ...
    batch_size = 64
    lr = 0.01
    training_validation_dataset = datasets.MNIST(path_cache, download=True, train=True, transform=transforms.ToTensor())
    ...
    test_dataset = datasets.MNIST(path_cache, train=False, transform=transforms.ToTensor())
    ...
    batch_size = 64
    lr = 0.01
    training_validation_dataset = datasets.FashionMNIST(path_cache, download=True, train=True, transform=transforms.ToTensor())
    ...
    test_dataset = datasets.FashionMNIST(path_cache, train=False, transform=transforms.ToTensor())
