...
if __name__ == '__main__':
    ...
    for index_dataset_name, (dataset_name, xlim_weights) in enumerate(zip(dataset_name_list, xlim_weights_list)):
        ...
        training_dataset = PhysionetDataset('training', dataset_name, cache_dir)
        training_dataloader = DataLoader(dataset=training_dataset, batch_size=batch_size, shuffle=True)
        validation_dataset = PhysionetDataset('validation', dataset_name, cache_dir)
        validation_dataloader = DataLoader(dataset=validation_dataset)
        test_dataset = PhysionetDataset('test', dataset_name, cache_dir)
        ...
        for index_sparse_activation, (sparse_activation, sparse_activation_color, sparse_activation_name) in enumerate(zip(sparse_activation_list, sparse_activation_color_list, sparse_activation_name_list)):
            ...
            save_images_1d(model_best, sparse_activation_name.lower().replace('_', '-').replace(' ', '-'), dataset_name, test_dataset[0][0][0], xlim_weights, results_dir)
        ...
        plt.savefig(f'{results_dir}/mean-inverse-compression-ratio-vs-mean-reconstruction-loss-variable-kernel-size-list-{dataset_name}')
        plt.close()
    ...
    df.to_latex(f'{results_dir}/mean-inverse-compression-ratio-mean-reconstruction-loss-variable-kernel-size.tex', bold_rows=True, escape=False, column_format='l|rrrr|rrrr|rrrr|rrrr|rrrr', multicolumn_format='c', formatters=formatters)
    ...
    plt.savefig(f'{results_dir}/mean-flithos-validation-epochs')
    plt.close()
    ...
    plt.grid(True)
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.savefig(f'{results_dir}/mean-flithos-variable-kernel-size-list')
    plt.close()
    ...
    plt.savefig(f'{results_dir}/mean-inverse-compression-ratio-vs-mean-reconstruction-loss-variable-kernel-size-list-legend')
    plt.close()
    ...
    plt.ylim([0, 2.5])
    plt.grid(True)
    plt.savefig(f'{results_dir}/crrl-density-plot')
    plt.close()

    print('UCI baseline, Supervised CNN classification')
    batch_size = 64
    lr = 0.01
    download_uci_epilepsy(cache_dir)
    training_dataset = UCIepilepsyDataset('training', cache_dir)
    ...
    validation_dataset = UCIepilepsyDataset('validation', cache_dir)
    ...
    test_dataset = UCIepilepsyDataset('test', cache_dir)
    ...
    for index_kernel_size_list, kernel_size_list in enumerate(kernel_size_list_list):
        ...
        for index_sparse_activation, (sparse_activation, sparse_activation_name) in enumerate(zip(sparse_activation_list, sparse_activation_name_list)):
            ...
            if kernel_size_list[0] == 10:
                save_images_1d(model_best, sparse_activation_name.lower().replace('_', '-').replace(' ', '-'), dataset_name, test_dataset[0][0][0], kernel_size_list[0], results_dir)
    ...
    df.to_latex(f'{results_dir}/uci-epilepsy-supervised.tex', bold_rows=True, escape=False, column_format='l|rrrr|rrrr|rrrr|rrrr|rrrr', multicolumn_format='c', formatters=formatters)

    print('MNIST baseline, Supervised FNN classification')
    batch_size = 64
    lr = 0.01
    training_validation_dataset = datasets.MNIST(cache_dir, download=True, train=True, transform=transforms.ToTensor())
    ...
    test_dataset = datasets.MNIST(cache_dir, train=False, transform=transforms.ToTensor())
    ...
    for index_kernel_size_list, kernel_size_list in enumerate(kernel_size_list_list):
        ...
        for index_sparse_activation, (sparse_activation, sparse_activation_name) in enumerate(zip(sparse_activation_list, sparse_activation_name_list)):
            ...
            if kernel_size_list[0] == 4:
                save_images_2d(model_best, sparse_activation_name.lower().replace('_', '-').replace(' ', '-'), test_dataset[0][0][0], dataset_name, results_dir)
    ...
    df.index.names = [r'$m$']
    formatters = 5*[lambda x: f'{x:.2f}', lambda x: f'{x:.2f}', lambda x: f'{x:.2f}', lambda x: f'{x:+.1f}']
    df.to_latex(f'{results_dir}/mnist-supervised.tex', bold_rows=True, escape=False, column_format='l|rrrr|rrrr|rrrr|rrrr|rrrr', multicolumn_format='c', formatters=formatters)

    print('FashionMNIST baseline, Supervised FNN classification')
    batch_size = 64
    lr = 0.01
    training_validation_dataset = datasets.FashionMNIST(cache_dir, download=True, train=True, transform=transforms.ToTensor())
    ...
    test_dataset = datasets.FashionMNIST(cache_dir, train=False, transform=transforms.ToTensor())
    ...
    df.index.names = [r'$m$']
    formatters = 5*[lambda x: f'{x:.2f}', lambda x: f'{x:.2f}', lambda x: f'{x:.2f}', lambda x: f'{x:+.1f}']
    df.to_latex(f'{results_dir}/fashionmnist-supervised.tex', bold_rows=True, escape=False, column_format='l|rrrr|rrrr|rrrr|rrrr|rrrr', multicolumn_format='c', formatters=formatters)
